 ...
 *
 * @details Abstracts away the creation and usage of a 2D texture. Textures are
 * 2D images that can be used to read and write data in the GPU. <br>
 *
 * staplegl provides a single interface for 2D texture creation whether the
 * texture is to be used as an object's material or as a framebuffer attachment.
 * <br>
 *
 ...
 * texture. <br>
 *
 * The constructor has sensible defaults, so that the user can create a simple
 * texture just by passing the data, the resolution and the color
 * format/filters.
 */
 ...
 * likely want to use `staplegl::tex_samples::MSAA_X1`.
 */
...
public:
   ...
   * immediately, every one of those instances should be overwritten before use
   * through a call to `texture_2d::(std::span<const float> data, resolution
   * res, texture_color color, bool generate_mipmap)`.
   *
   ...
   * example, would be practically equivalent to unbinding the color attachment
   * for that particular slot).
   *
   * @warning this constructor does not initialize the texture object, remember
   * to perform initialization before using it.
   */
  texture_2d() = default;
   ...
   * defaults to false.
   */
  texture_2d(std::span<const float> data, resolution res, texture_color color,
             texture_filter filters, tex_samples samples = tex_samples::MSAA_X1,
             bool generate_mipmap = false, bool init_storage = true) noexcept;
  ...
  texture_2d(texture_2d &&other) noexcept
      : m_id(other.m_id), m_unit(other.m_unit), m_color(other.m_color),
        m_resolution(other.m_resolution), m_antialias(other.m_antialias) {
    other.m_id = 0;
  }
   ...
   *
   */
  void bind() const { glBindTexture(m_antialias.type, m_id); }

  /**
   * @brief Unbind the texture object.
   *
   */
  void unbind() const { glBindTexture(m_antialias.type, 0); }
  /**
   * @brief Get the last texture unit this texture was bound to.
   *
   * @return uint32_t a texture unit offset from `GL_TEXTURE0`.
   */
  [[nodiscard]] auto constexpr get_unit() const -> uint32_t { return m_unit; }
   ...
   * @return std::uint32_t
   */
  [[nodiscard]] constexpr auto id() const -> std::uint32_t { return m_id; }
...
inline texture_2d::texture_2d(std::span<const float> data, resolution res,
                              texture_color color, texture_filter filter,
                              tex_samples samples,
                              bool generate_mipmap, bool init_storage) noexcept
    : m_color{color}, m_filter{filter}, m_resolution{res},
      m_antialias{(samples == tex_samples::MSAA_X1)
                      ? texture_antialias{GL_TEXTURE_2D, samples}
                      : texture_antialias{GL_TEXTURE_2D_MULTISAMPLE, samples}} {
  glGenTextures(1, &m_id);
  glBindTexture(m_antialias.type, m_id);

  if (m_antialias.type == GL_TEXTURE_2D) {
    glTexParameteri(m_antialias.type, GL_TEXTURE_MIN_FILTER,
                    generate_mipmap ? to_mipmap(filter.min_filter)
                                    : filter.min_filter);
    ...
    if(init_storage) {
      glTexImage2D(m_antialias.type, 0, color.internal_format, m_resolution.width,
                  m_resolution.height, 0, color.format, color.datatype,
                  data.data());
    }
...
inline void texture_2d::set_data(std::span<const float> data, resolution res,
  ...
  glTexImage2D(m_antialias.type, 0, m_color.internal_format, res.width,
               res.height, 0, m_color.format, m_color.datatype, data.data());
  m_color = color;
  m_resolution = res;
