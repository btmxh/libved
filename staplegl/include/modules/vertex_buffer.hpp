...
enum driver_access_specifier {
    READ_ONLY = GL_MAP_READ_BIT,
    WRITE_ONLY = GL_MAP_WRITE_BIT,
...
void vertex_buffer::apply(const std::function<void(std::span<T> vertices)>& func, driver_access_specifier access_specifier) noexcept
{
    ...
    func(std::span { reinterpret_cast<T*>(glMapBufferRange(GL_ARRAY_BUFFER, 0, buffer_size, access_specifier)), // NOLINT (reinterpret-cast)
